...
const VolunteerForm: React.FC = () => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    } else if (type === 'checkbox') { // For other checkboxes like consent
      setFormData({
        ...formData,
        [name]: checked,
      });
  ...
  const validateForm = (step: number): boolean => {
    ...
    if (step === 1) { // Validation for Basic Information & Interest
    ...
    } else if (step === 2) { // Validation for Skills & Experience
      ...
      if (formData.primaryInterest === 'Content Creation') {
        ...
        if (!formData.techGithub?.trim()) {
          currentErrors.techGithub = 'Please provide your GitHub profile or project links.';
          isValid = false;
        }
      ...
      } else if (formData.primaryInterest === 'Moderation/Community Management') {
        if (!formData.moderationExperience?.trim()) {
          currentErrors.moderationExperience = 'Please describe your moderation experience.';
          isValid = false;
        }
    ...
    } else if (step === 3) { // Validation for Understanding & Alignment
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (validateForm(currentStep)) {
      ...
      try {
        const GOOGLE_APPS_SCRIPT_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycby7yNxUs4z34Vd3_Szl98Bu4MScIh7PxeMbf3_KN0JBYcFC9YGO7RRE_rKvDDt10iXK/exec';
        ...
        if (result.status === 'ERROR') {
          throw new Error(result.message || 'Form submission failed with an unknown error.');
        }
        ...
        setFormData({ // Reset form for new submission
          fullName: '',
          email: '',
          phone: '',
          location: '',
      ...
      } catch (error) { // Use 'any' for error type if not specific
        console.error('Submission error:', error);
        // Display a general error message to the user
        setErrors({ general: 'There was an error submitting your application. Please try again.' });
      } finally {
  ...
  const inputClassName = `form-input flex w-full px-4 mt-1 mb-5 rounded-lg text-teal-900 border border-teal-200 focus:ring-teal-500 focus:border-teal-500 h-[50px] !py-1`;
      ...
      <div className="max-w-3xl w-full bg-white shadow-xl rounded-2xl p-8 space-y-8">
        ...
        {submissionSuccess ? (
          <div className="text-center p-6 bg-teal-50 border border-teal-200 rounded-lg">
            <h2 className="text-2xl font-bold text-teal-700 mb-3">Jazakallahu Khoyron! 🎉</h2>
            <p className="text-teal-800">Your application has been successfully submitted. We appreciate your interest and will get back to you soon.</p>
            <button
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {currentStep === 1 && (
              <div className="p-6 bg-gray-50 rounded-lg shadow-sm">
                <h2 className="text-2xl font-bold text-teal-800 mb-6 text-center">Step 1: Your Details & Interest</h2>
                <p className="text-gray-600 mb-6 text-center">Let's start with some basic information about you and your areas of interest.</p>

                {/* Full Name */}
                <div>
                  <label htmlFor="fullName" className="block text-teal-900 font-medium mb-2">
                ...
                {/* Phone Number */}
                <div>
                  <label htmlFor="phone" className="block text-teal-900 font-medium mb-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      className={`${inputClassName} ${errors.location ? 'border-red-500' : ''}`}
                  ...
                  <div>
                    <label htmlFor="country" className="block text-teal-900 font-medium mb-2">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className={`${inputClassName} ${errors.gender ? 'border-red-500' : ''}`}
                  >
                ...
                {/* Age Group */}
                <div>
                  <label htmlFor="ageGroup" className="block text-teal-900 font-medium mb-2">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className={`${inputClassName} ${errors.ageGroup ? 'border-red-500' : ''}`}
                  >
                  ...
                  {errors.ageGroup && (
                    <p className="mt-1 text-red-600 text-sm">{errors.ageGroup}</p>
                  )}
                </div>
                ...
                <div>
                  <label className="block text-teal-900 font-medium mb-2">
                    Which Team(s) are you interested in joining? <span className="text-red-500">*</span>
                  </label>
                  ...
                  {errors.teamInterest && (
                    <p className="mt-1 text-red-600 text-sm">{errors.teamInterest}</p>
                  )}
                </div>
                ...
                {formData.teamInterest.length > 0 && (
                  <div>
                    <label htmlFor="primaryInterest" className="block text-teal-900 font-medium mt-4 mb-2">
                      From your selections above, which team is your *primary* interest? <span className="text-red-500">*</span>
                    </label>
                    <select
                      id="primaryInterest"
                      ...
                      onChange={handleChange}
                      className={`${inputClassName} ${errors.primaryInterest ? 'border-red-500' : ''}`}
                    >
                    ...
                    {errors.primaryInterest && (
                      <p className="mt-1 text-red-600 text-sm">{errors.primaryInterest}</p>
                    )}
                  </div>
                )}
            ...
            {currentStep === 2 && (
              <div className="p-6 bg-gray-50 rounded-lg shadow-sm">
                <h2 className="text-2xl font-bold text-teal-800 mb-6 text-center">Step 2: Your Skills & Experience</h2>
                <p className="text-gray-600 mb-6 text-center">Tell us more about your relevant experience for your chosen team.</p>

                {/* Years of Experience */}
                <div>
                  <label htmlFor="yearsExperience" className="block text-teal-900 font-medium mb-2">
                    Years of professional or significant volunteer experience <span className="text-red-500">*</span>
                  </label>
                  <select
                    id="yearsExperience"
                    ...
                    onChange={handleChange}
                    className={`${inputClassName} ${errors.yearsExperience ? 'border-red-500' : ''}`}
                  >
                  ...
                  {errors.yearsExperience && (
                    <p className="mt-1 text-red-600 text-sm">{errors.yearsExperience}</p>
                  )}
                </div>
                ...
                {formData.primaryInterest === 'Content Creation' && (
                  <>
                    <div>
                      <label htmlFor="contentSkills" className="block text-teal-900 font-medium mb-2">
                        Please describe your content creation skills. <span className="text-red-500">*</span>
                      </label>
                      <textarea
                        id="contentSkills"
                        name="contentSkills"
                        ...
                        onChange={handleChange}
                        className={`${inputClassName} h-24 ${errors.contentSkills ? 'border-red-500' : ''}`}
                        placeholder="e.g., SEO writing, storytelling, editing, proofreading..."
                      ></textarea>
                      {errors.contentSkills && (
                        <p className="mt-1 text-red-600 text-sm">{errors.contentSkills}</p>
                      )}
                    </div>
                    <div>
                      <label htmlFor="writingSamples" className="block text-teal-900 font-medium mb-2">
                        Please provide links to your writing samples/portfolio. <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="url"
                        id="writingSamples"
                        ...
                        onChange={handleChange}
                        className={`${inputClassName} ${errors.writingSamples ? 'border-red-500' : ''}`}
                        placeholder="e.g., yourwebsite.com/articles or drive.google.com/..."
                      />
                      {errors.writingSamples && (
                        <p className="mt-1 text-red-600 text-sm">{errors.writingSamples}</p>
                      )}
                    </div>
                  </>
                )}
                ...
                {formData.primaryInterest === 'Design' && (
                  <>
                    <div>
                      <label htmlFor="designSoftware" className="block text-teal-900 font-medium mb-2">
                        Which design software/tools do you use? <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="text"
                        id="designSoftware"
                        ...
                        onChange={handleChange}
                        className={`${inputClassName} ${errors.designSoftware ? 'border-red-500' : ''}`}
                        placeholder="e.g., Figma, Adobe Photoshop, Illustrator, Canva"
                      />
                      {errors.designSoftware && (
                        <p className="mt-1 text-red-600 text-sm">{errors.designSoftware}</p>
                      )}
                    </div>
                    <div>
                      <label htmlFor="designPortfolio" className="block text-teal-900 font-medium mb-2">
                        Please provide a link to your design portfolio. <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="url"
                        id="designPortfolio"
                        ...
                        onChange={handleChange}
                        className={`${inputClassName} ${errors.designPortfolio ? 'border-red-500' : ''}`}
                        placeholder="e.g., behance.net/yourname or yourportfolio.com"
                      />
                      {errors.designPortfolio && (
                        <p className="mt-1 text-red-600 text-sm">{errors.designPortfolio}</p>
                      )}
                    </div>
                  </>
                )}
                ...
                {formData.primaryInterest === 'Tech/Development' && (
                  <>
                    <div>
                      <label htmlFor="techLanguages" className="block text-teal-900 font-medium mb-2">
                        Which programming languages/frameworks are you proficient in? <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="text"
                        id="techLanguages"
                        ...
                        onChange={handleChange}
                        className={`${inputClassName} ${errors.techLanguages ? 'border-red-500' : ''}`}
                        placeholder="e.g., JavaScript, React, Python, Node.js, Firebase"
                      />
                      {errors.techLanguages && (
                        <p className="mt-1 text-red-600 text-sm">{errors.techLanguages}</p>
                      )}
                    </div>
                    <div>
                      <label htmlFor="techGithub" className="block text-teal-900 font-medium mb-2">
                        Please provide your GitHub profile or relevant project links. <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="url"
                        id="techGithub"
                        ...
                        onChange={handleChange}
                        className={`${inputClassName} ${errors.techGithub ? 'border-red-500' : ''}`}
                        placeholder="e.g., github.com/yourusername or link to a deployed project"
                      />
                      {errors.techGithub && (
                        <p className="mt-1 text-red-600 text-sm">{errors.techGithub}</p>
                      )}
                    </div>
                  </>
                )}
                ...
                {formData.primaryInterest === 'Public Relations' && (
                  <div>
                    <label htmlFor="prExperience" className="block text-teal-900 font-medium mb-2">
                      Please describe your experience in Public Relations or Community Outreach. <span className="text-red-500">*</span>
                    </label>
                    <textarea
                      id="prExperience"
                      name="prExperience"
                      ...
                      onChange={handleChange}
                      className={`${inputClassName} h-24 ${errors.prExperience ? 'border-red-500' : ''}`}
                      placeholder="e.g., social media management, partnership building, event coordination..."
                    ></textarea>
                    {errors.prExperience && (
                      <p className="mt-1 text-red-600 text-sm">{errors.prExperience}</p>
                    )}
                  </div>
                )}
                ...
                {formData.primaryInterest === 'Moderation/Community Management' && (
                  <div>
                    <label htmlFor="moderationExperience" className="block text-teal-900 font-medium mb-2">
                      Please describe your experience with online moderation or community management. <span className="text-red-500">*</span>
                    </label>
                    <textarea
                      ...
                      onChange={handleChange}
                      className={`${inputClassName} h-24 ${errors.moderationExperience ? 'border-red-500' : ''}`}
                      placeholder="e.g., forum moderation, conflict resolution, user support..."
                    ></textarea>
                    {errors.moderationExperience && (
                      <p className="mt-1 text-red-600 text-sm">{errors.moderationExperience}</p>
                    )}
                  </div>
                )}
                ...
                {formData.primaryInterest === 'Other' && (
                  <div>
                    <label htmlFor="otherSkills" className="block text-teal-900 font-medium mb-2">
                      Please specify your other skills and how they relate to Baytul-Sakinah. <span className="text-red-500">*</span>
                    </label>
                    <textarea
                      id="otherSkills"
                      name="otherSkills"
                      ...
                      onChange={handleChange}
                      className={`${inputClassName} h-24 ${errors.otherSkills ? 'border-red-500' : ''}`}
                      placeholder="Describe your skills here..."
                    ></textarea>
                    {errors.otherSkills && (
                      <p className="mt-1 text-red-600 text-sm">{errors.otherSkills}</p>
                    )}
                  </div>
                )}
            ...
            {currentStep === 3 && (
              <div className="p-6 bg-gray-50 rounded-lg shadow-sm">
                <h2 className="text-2xl font-bold text-teal-800 mb-6 text-center">Step 3: Understanding & Alignment</h2>
                <p className="text-gray-600 mb-6 text-center">Help us understand your connection to our mission and values.</p>

                {/* Motivation */}
                <div>
                  <label htmlFor="motivation" className="block text-teal-900 font-medium mb-2">
                    What is your primary motivation for joining with Baytul-Sakinah? <span className="text-red-500">*</span>
                  </label>
                  <textarea
                    id="motivation"
                    name="motivation"
                    ...
                    onChange={handleChange}
                    className={`${inputClassName} h-20 ${errors.motivation ? 'border-red-500' : ''}`}
                  ...
                  ></textarea>
                  {errors.motivation && (
                    <p className="mt-1 text-red-600 text-sm">{errors.motivation}</p>
                  )}
                </div>

                {/* Islamic Values Alignment */}
                <div>
                  <label htmlFor="islamicValues" className="block text-teal-900 font-medium mb-2">
                    Baytul-Sakinah upholds Islamic values (e.g., modesty, privacy, no gossip). How do you envision contributing to and upholding these values?
                  </label>
                  <textarea
                    id="islamicValues"
                    name="islamicValues"
                ...
                <div>
                  <label htmlFor="challenges" className="block text-teal-900 font-medium mb-2">
                    What do you believe are some of the biggest challenges facing young Muslim couples in Nigeria today, and how do you think Baytul-Sakinah can help address them?
                  </label>
                  <textarea
                    id="challenges"
                    name="challenges"
                ...
                <div>
                  <label htmlFor="howDidYouHear" className="block text-teal-900 font-medium mb-2">
                  ...
                  >
                    ...
                    <option value="Mosque/Islamic Organization">Mosque/Islamic Organization</option>
                ...
                <div className="mt-6">
                  <label className="inline-flex items-center">
                    <input
                      ...
                      className="form-checkbox text-teal-600 rounded-md"
                    />
                    <span className="ml-2 text-teal-900">I confirm that the information provided in this application is accurate and true to the best of my knowledge. <span className="text-red-500">*</span></span>
                  </label>
                  {errors.consent && (
                    <p className="mt-1 text-red-600 text-sm">{errors.consent}</p>
                  )}
                </div>

                {errors.general && (
                  <p className="mt-4 text-red-600 text-center">{errors.general}</p>
                )}
